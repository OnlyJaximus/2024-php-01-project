<?php
require("../config/function.php");

if (isset($_POST['saveUser'])) {

    $name =  mysqli_real_escape_string($conn, $_POST['name']);
    $phone =  validate($_POST['phone']);
    $email =  validate($_POST['email']);
    $password =  validate($_POST['password']);

    // $is_ban =  validate($_POST['is_ban']) == true ? 1 : 0;
    $is_ban =  isset($_POST['is_ban']) == true ? 1 : 0;

    $role =  validate($_POST['role']);


    if ($name != '' || $phone != '' || $email != '' || $password != '') {

        $hashedPassword = password_hash($password, PASSWORD_BCRYPT);

        // print_r($hashedPassword);
        // die;

        $data = [
            'name' => $name,
            'phone' => $phone,
            'email' => $email,
            'password' => $hashedPassword,
            'is_ban' => $is_ban,
            'role' => $role,
        ];

        $result = insert('users', $data);
        // echo $result;  // name,phone,email,password,is_ban,role



        // Drugi Nacin
        //         $query = "INSERT INTO users (`name`, `phone`, `email`, `password`, `is_ban`, `role`) 
        // VALUES ('$name', '$phone', '$email','$hashedPassword', '$is_ban', '$role')";
        //         $results = mysqli_query($conn, $query);

        if ($result) {
            redirect('users.php', "User or Admin Added Successfully");
        } else {
            redirect('users-create.php', "Something Went Wrong :-/");
        }
    } else {
        redirect("users-create.php", "Please, fill all the input fields!");
    }
}


// Update user
if (isset($_POST['updateUser'])) {



    $name =  mysqli_real_escape_string($conn, $_POST['name']);
    $phone =  validate($_POST['phone']);
    $email =  validate($_POST['email']);
    $password =  validate($_POST['password']);

    // $is_ban =  validate($_POST['is_ban']) == true ? 1 : 0;
    $is_ban =  isset($_POST['is_ban']) == true ? 1 : 0;

    $role =  validate($_POST['role']);

    $userId = validate($_POST['userId']);

    $user = getById('users', $userId);

    if ($user['status'] != 200) {

        redirect('users-edit.php?id=' . $userId, "No Such id Found! :-/");
    }

    if ($name != '' || $phone != '' || $email != '' || $password != '') {

        $hashedPassword = password_hash($password, PASSWORD_BCRYPT);

        $data = [
            'name' => $name,
            'phone' => $phone,
            'email' => $email,
            'password' => $hashedPassword,
            'is_ban' => $is_ban,
            'role' => $role

        ];

        $result = update('users', $userId, $data);
        //echo $result;  // test

        // name='Admin Draza',phone='064333555',email='admin@yahoo.com',password='$2y$10$5pRENaGOUu.FzriIoUs9kONTHlpGPfkqUwPTpUpBuZL88xLWPVyWm',is_ban='0',role='admin'

        // 2 Nacin  ********************************
        // $query = "UPDATE users SET 
        // name = '$name',
        // phone = '$phone',
        // email= '$email', 
        // password = '$hashedPassword', 
        // is_ban = '$is_ban', 
        // role = '$role'
        // WHERE id = '$userId' ";
        // $result = mysqli_query($conn, $query);

        if ($result) {
            redirect('users.php', "User or Admin Updated Successfully");
        } else {
            redirect('users-edit.php?id=' . $userId, "Something Went Wrong :-/");
        }
    } else {
        redirect("users-create.php", "Please, fill all the input fields!");
    }
}

// Setting 
if (isset($_POST['saveSetting'])) {
    $title = validate($_POST['title']);
    $slug = validate($_POST['slug']);


    $small_description = validate($_POST['small_description']);
    $meta_description = validate($_POST['meta_description']);

    $meta_keyword = validate($_POST['meta_keyword']);

    $email1 = validate($_POST['email1']);
    $email2 = validate($_POST['email2']);
    $phone1 = validate($_POST['phone1']);
    $phone2 = validate($_POST['phone2']);
    $address = validate($_POST['address']);

    $settingId = validate($_POST['settingId']);

    if ($settingId == 'insert') {

        $query = "INSERT INTO `settings`(`title`, `slug`, `small_description`, `meta_description`, `meta_keyword`, `email1`, `email2`, `phone1`, `phone2`, `address`) 
        VALUES ('$title','$slug','$small_description','$meta_description','$meta_keyword','$email1','$email2',' $phone1',' $phone2', '$address')";

        $result = mysqli_query($conn, $query);
    }

    if (is_numeric($settingId)) {
        $query = "UPDATE settings SET
            title = '$title',
            slug = '$slug',
            small_description ='$small_description',
            meta_description = '$meta_description',
            meta_keyword = '$meta_keyword',
            email1 = '$email1',
            email2 = '$email2',
            phone1 = '$phone1',
            phone2 = '$phone2',
            address = '$address' WHERE id = '$settingId' ";

        $result = mysqli_query($conn, $query);
    }


    if ($result) {
        redirect('settings.php', 'Setting Saved');
    } else {
        redirect('settings.php', 'Someting Went Wrong!');
    }
}

// Create Social Media
if (isset($_POST['saveSocialMedia'])) {

    $name =  mysqli_real_escape_string($conn, $_POST['name']);
    $url =  validate($_POST['url']);
    $status =  validate($_POST['status']) == true ? 1 : 0;



    if ($name != '' || $url != '') {
        $query = "INSERT INTO socail_medias(`name`, `url`,`status`) 
        VALUES ('$name', '$url', '$status')";
        $results = mysqli_query($conn, $query);

        if ($results) {
            redirect('social-media.php', "Social Media Added Successfully");
        } else {
            redirect('social-media-create.php', "Something Went Wrong :-/");
        }
    } else {
        redirect("social-media-create.php", "Please, fill all the input fields!");
    }
}

// Update Socail Media
if (isset($_POST['updateSocialMedia'])) {
    $name =  mysqli_real_escape_string($conn, $_POST['name']);
    $url =  validate($_POST['url']);
    $status =  validate($_POST['status']) == true ? 1 : 0;

    $socialMediaId = validate($_POST['socialMediaId']);



    if ($name != '' || $url != '') {
        $query = "UPDATE  socail_medias SET `name` = '$name',`url`='$url',`status`=$status WHERE id='$socialMediaId' LIMIT 1";

        $results = mysqli_query($conn, $query);

        if ($results) {
            redirect('social-media.php', "Social Media Updated Successfully");
        } else {
            redirect('social-media-edit.php?id=' . $socialMediaId, "Something Went Wrong :-/");
        }
    } else {
        redirect("social-media-edit.php?id='.$socialMediaId", "Please, fill all the input fields!");
    }
}


// Service Create
if (isset($_POST['saveService'])) {

    $name = validate($_POST['name']);

    // space zamenjujemo sa - i sve stavljamo lower case
    $slug = str_replace(' ', '-', strtolower($name));


    $small_description = validate($_POST['small_description']);
    $long_description = validate($_POST['long_description']);


    if ($_FILES['image']['size'] > 0) {

        $image =  $_FILES['image']['name'];

        $imgFileType = strtolower(pathinfo($image, PATHINFO_EXTENSION));

        if ($imgFileType != 'jpg' && $imgFileType != 'jpeg' && $imgFileType != 'png') {
            redirect('services.php', "Sorry, only JPG, JPEG, PNG images only!");
        }

        $path = "assets/uploads/services/";
        $imgExt = pathinfo($image, PATHINFO_EXTENSION);
        $filename = time() . '.' . $imgExt;
        $finalImage = 'assets/uploads/services/' . $filename;
    } else {

        $finalImage = NULL;
    }


    $meta_title = validate($_POST['meta_title']);
    $meta_description = validate($_POST['meta_description']);
    $meta_keyword = validate($_POST['meta_keyword']);


    $status = validate($_POST['status']) == true ? '1' : '0';

    $query = "INSERT INTO `services`
    (`name`, `slug`, `small_description`, `long_description`,`image`, `meta_title`, `meta_description`, `meta_keyword`, `status`)
    VALUES ('$name', '$slug', '$small_description', '$long_description',
 '$finalImage', '$meta_title', '$meta_description', '$meta_keyword', '$status')";

    $result = mysqli_query($conn, $query);

    if ($result) {
        if ($_FILES['image']['size'] > 0) {
            move_uploaded_file($_FILES['image']['tmp_name'], $path . $filename);
        }

        redirect('services.php', "Services Added Successfully");
    } else {
        redirect('services.php', "Something Went Wrong!");
    }
}



//  services update
if (isset($_POST['updateService'])) {

    $serviceId = validate($_POST['serviceId']);
    $name = validate($_POST['name']);

    // space zamenjujemo sa - i sve stavljamo lower case
    $slug = str_replace(' ', '-', strtolower($name));


    $small_description = validate($_POST['small_description']);
    $long_description = validate($_POST['long_description']);

    $service = getById('services', $serviceId);


    if ($_FILES['image']['size'] > 0) {

        $image =  $_FILES['image']['name'];

        $imgFileType = strtolower(pathinfo($image, PATHINFO_EXTENSION));

        if ($imgFileType != 'jpg' && $imgFileType != 'jpeg' && $imgFileType != 'png') {
            redirect('services.php', "Sorry, only JPG, JPEG, PNG images only!");
        }


        // $service['data']['image'] = u bazi podataka assets/uploads/services/1706809202.jpg
        $deleteImage = $service['data']['image'];

        $path = 'assets/uploads/services/';

        if (file_exists($deleteImage)) {
            unlink($deleteImage);
        }

        // $path = "assets/uploads/services/";
        $imgExt = pathinfo($image, PATHINFO_EXTENSION);
        $filename = time() . '.' . $imgExt;
        $finalImage = 'assets/uploads/services/' . $filename;
    } else {

        $finalImage = $service['data']['image'];
    }


    $meta_title = validate($_POST['meta_title']);
    $meta_description = validate($_POST['meta_description']);
    $meta_keyword = validate($_POST['meta_keyword']);


    $status = validate($_POST['status']) == true ? '1' : '0';

    $query = "UPDATE services SET
    name = '$name',
    slug = '$slug',
    small_description = '$small_description',
    long_description = '$long_description',
    image = '$finalImage',
    meta_title = '$meta_title',
    meta_description = '$meta_description', 
    meta_keyword = '$meta_keyword', 
    status = '$status'
    WHERE id = '$serviceId' ";

    $result = mysqli_query($conn, $query);

    if ($result) {
        if ($_FILES['image']['size'] > 0) {
            move_uploaded_file($_FILES['image']['tmp_name'], $path . $filename);
        }
        redirect('services-edit.php?id=' . $serviceId, "Services Updated Successfully");
    } else {
        redirect('services-edit.php?id=' . $serviceId, "Something Went Wrong!");
    }
}


// Update Enquiry Status
if (isset($_POST['updateEnquiryStatus'])) {

    $enquiryId = validate($_POST['enquiryId']);
    $status = validate($_POST['status']);

    $query = "UPDATE enquires SET `status`= '$status' WHERE `id`= '$enquiryId' ";
    $result = mysqli_query($conn, $query);

    if ($result) {
        redirect('enquires-view.php?id=' . $enquiryId, "Enquiry  Status Updated!");
    } else {
        redirect('enquires-view.php?id=' . $enquiryId, "Something Went Wrong!");
    }
}
